...
LHPlucked::LHPlucked(mrs_string name) : MarSystem("LHPlucked", name)
{
...
/* //from template
LHPlucked::LHPlucked(const LHPlucked& a) : MarSystem(a)
{
...
MarSystem*
LHPlucked::clone() const
{
	return new LHPlucked(*this);
}

void
LHPlucked::addControls()
{
	/*///From template:  Add any specific controls needed by this MarSystem.
...
void
LHPlucked::myUpdate(MarControlPtr sender)
{
	MRSDIAG("LHPlucked.cpp - LHPlucked:myUpdate");
	...
	/*
  	...
  	pos = pluck position along waveguide (0 < p < 1 - fraction of waveguide length)
	*/

	mrs_real fs = getctrl("mrs_real/israte")->to<mrs_real>();
	mrs_real length = getctrl("mrs_real/length")->to<mrs_real>();
...
void 
LHPlucked::myProcess(realvec &in, realvec &out)
{
  ...
  if (noteon_ > 0)
	...
	//delayline1_(rp_) =loss_*(B1[0]*a_ + B2[0]* b_); //feed-forward of loop filter

	//need feed back og loop filter using A1 and A2
